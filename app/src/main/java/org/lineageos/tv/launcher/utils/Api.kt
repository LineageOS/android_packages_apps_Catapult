...
import org.lineageos.tv.launcher.ext.executeAsync
...
suspend fun <T, O> MethodResult<T>.toRequestStatus(
...
): RequestStatus<O, ApiError> = when (this) {
    ...
    is MethodResult.HttpError -> RequestStatus.Error(
        when (code) {
            ...
            403 -> ApiError.INVALID_CREDENTIALS
    ...
    is MethodResult.DeserializationError -> RequestStatus.Error(ApiError.DESERIALIZATION, error)
    is MethodResult.CancellationError -> RequestStatus.Error(ApiError.CANCELLED, error)
    is MethodResult.InvalidResponse -> RequestStatus.Error(ApiError.INVALID_RESPONSE, error)
    is MethodResult.GenericError -> RequestStatus.Error(ApiError.IO, error)
}
